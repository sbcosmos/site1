 ...
 */
function fashion_pin_theme_setup(){
    /*
     * Make chile theme available for translation.
     * Translations can be filed in the /languages/ directory.
     */
    load_child_theme_textdomain( 'fashion-pin', get_stylesheet_directory() . '/languages' );

    add_image_size( 'fashion-pin-slider-two', 1903, 660, true );

}
add_action( 'after_setup_theme', 'fashion_pin_theme_setup', 100 );

function fashion_pin_customize_script(){

    $my_theme = wp_get_theme();
    $version = $my_theme['Version'];
    wp_enqueue_script('fashion-pin-customize', get_stylesheet_directory_uri() . '/js/child-customize.js', array('jquery', 'customize-controls'), $version, true);

}
add_action('customize_controls_enqueue_scripts', 'fashion_pin_customize_script');

 function fashion_pin_enqueue_styles() {
    $my_theme = wp_get_theme();
    $version = $my_theme['Version'];
   
    wp_enqueue_style( 'blossom-pin', get_template_directory_uri() . '/style.css' );
    wp_enqueue_style( 'fashion-pin', get_stylesheet_directory_uri() . '/style.css', array( 'blossom-pin' ), $version );

    wp_enqueue_script( 'fashion-pin', get_stylesheet_directory_uri() . '/js/custom.js', array('jquery' ), $version, true );

    $array = array( 
        'rtl'       => is_rtl(),
    ); 
    wp_localize_script( 'fashion-pin', 'fashion_pin_data', $array );

}
add_action( 'wp_enqueue_scripts', 'fashion_pin_enqueue_styles', 10 );

//Remove a function from the parent theme
function fashion_pin_remove_parent_filters(){ //Have to do it after theme setup, because child theme functions are loaded first
...
add_action( 'init', 'fashion_pin_remove_parent_filters' );

function fashion_pin_customizer_register( $wp_customize ) {
    $wp_customize->add_section( 'theme_info', array(
        'title'       => __( 'Demo & Documentation' , 'fashion-pin' ),
        'priority'    => 6,
    ) );
    ...
    $theme_info = '<p>';
    $theme_info .= sprintf( __( 'Demo Link: %1$sClick here.%2$s', 'fashion-pin' ),  '<a href="' . esc_url( 'https://blossomthemes.com/theme-demo/?theme=fashion-pin' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p><p>';
    $theme_info .= sprintf( __( 'Documentation Link: %1$sClick here.%2$s', 'fashion-pin' ),  '<a href="' . esc_url( 'https://docs.blossomthemes.com/docs/fashion-pin/' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p>';
    ...
    $wp_customize->add_control( 
        new WP_Customize_Color_Control( 
            ...
            'primary_color', 
            array(
                'label'       => __( 'Primary Color', 'fashion-pin' ),
                'description' => __( 'Primary color of the theme.', 'fashion-pin' ),
                'section'     => 'colors',
                'priority'    => 5,                
            )
        )
    ...
    $wp_customize->add_panel( 
        'appearance_settings',
         array(
            ...
            'title'       => __( 'Appearance Settings', 'fashion-pin' ),
            'description' => __( 'Customize Typography & Background Image', 'fashion-pin' ),
        ) 
    );
    ...
    $wp_customize->add_section(
        'typography_settings',
        array(
            'title'    => __( 'Typography', 'fashion-pin' ),
            'priority' => 15,
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Select_Control(
            ...
            'primary_font',
            array(
                'label'       => __( 'Primary Font', 'fashion-pin' ),
                'description' => __( 'Primary font of the site.', 'fashion-pin' ),
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Select_Control(
            ...
            'secondary_font',
            array(
                'label'       => __( 'Secondary Font', 'fashion-pin' ),
                'description' => __( 'Secondary font of the site.', 'fashion-pin' ),
                'section'     => 'typography_settings',
                'choices'     => blossom_pin_get_all_fonts(),   
            )
        )
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Slider_Control( 
            ...
            array(
                'section'     => 'typography_settings',
                'label'       => __( 'Font Size', 'fashion-pin' ),
                'description' => __( 'Change the font size of your site.', 'fashion-pin' ),
                'choices'     => array(
                    'min'   => 10,
                    'max'   => 50,
                    'step'  => 1,
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Toggle_Control( 
            ...
            array(
                'section'       => 'typography_settings',
                'label'         => __( 'Load Google Fonts Locally', 'fashion-pin' ),
                'description'   => __( 'Enable to load google fonts from your own server instead from google\'s CDN. This solves privacy concerns with Google\'s CDN and their sometimes less-than-transparent policies.', 'fashion-pin' )
            )
        )
    );   
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Toggle_Control( 
            ...
            array(
                'section'       => 'typography_settings',
                'label'         => __( 'Preload Local Fonts', 'fashion-pin' ),
                'description'   => __( 'Preloading Google fonts will speed up your website speed.', 'fashion-pin' ),
    ...
    ob_start(); ?>
        ...
        <span style="margin-bottom: 5px;display: block;"><?php esc_html_e( 'Click the button to reset the local fonts cache', 'fashion-pin' ); ?></span>
        
        <input type="button" class="button button-primary blossom-pin-flush-local-fonts-button" name="blossom-pin-flush-local-fonts-button" value="<?php esc_attr_e( 'Flush Local Font Files', 'fashion-pin' ); ?>" />
    <?php
    $fashion_pin_flush_button = ob_get_clean();
    ...
    $wp_customize->add_control(
        'ed_flush_local_fonts',
        array(
            'label'         => __( 'Flush Local Fonts Cache', 'fashion-pin' ),
            'section'       => 'typography_settings',
            'description'   => $fashion_pin_flush_button,
            'type'          => 'hidden',
            'active_callback' => 'blossom_pin_ed_localgoogle_fonts'
        )
    );
    ...
    $wp_customize->add_section(
        'header_layout',
        array(
            'title'    => __( 'Header Layout', 'fashion-pin' ),
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Header Layout', 'fashion-pin' ),
                'description' => __( 'it is the layout for header.', 'fashion-pin' ),
                'choices'     => array(                 
    ...
    $wp_customize->add_section(
        ...
        array(
            'title'    => __( 'Slider Layout', 'fashion-pin' ),
            'priority' => 20,
            'panel'    => 'layout_settings',
        )
    );
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Slider Layout', 'fashion-pin' ),
                'description' => __( 'Choose the layout of the slider for your site.', 'fashion-pin' ),
                'choices'     => array(
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Note_Control( 
            ...
            array(
                'section'     => 'slider_layout_settings',
                'description' => sprintf( __( '%1$sClick here%2$s to configure slider settings.', 'fashion-pin' ), '<span class="text-inner-link slider_banner_layout_text">', '</span>' ),
            )
        )
    );
    ...
    $wp_customize->add_control(
        new Blossom_Pin_Note_Control( 
            ...
            array(
                ...
                'description' => sprintf( __( '%1$sClick here%2$s to select the layout of slider banner.', 'fashion-pin' ), '<span class="text-inner-link slider_banner_text">', '</span>' ),
            )
        )
    );
}
add_action( 'customize_register', 'fashion_pin_customizer_register', 40);
        ...
        <div class="tools">
            ...
            if( $ed_header_search ) : ?>
                <div class=header-search>
                    <button aria-label="<?php esc_attr_e( 'search form toggle', 'fashion-pin' ); ?>" class="search-icon search-toggle" data-toggle-target=".search-modal" data-toggle-body-class="showing-search-modal" data-set-focus=".search-modal .search-field" aria-expanded="false">
                        <svg class="open-icon" xmlns="http://www.w3.org/2000/svg" viewBox="-18214 -12091 18 18"><path id="Path_99" data-name="Path 99" d="M18,16.415l-3.736-3.736a7.751,7.751,0,0,0,1.585-4.755A7.876,7.876,0,0,0,7.925,0,7.876,7.876,0,0,0,0,7.925a7.876,7.876,0,0,0,7.925,7.925,7.751,7.751,0,0,0,4.755-1.585L16.415,18ZM2.264,7.925a5.605,5.605,0,0,1,5.66-5.66,5.605,5.605,0,0,1,5.66,5.66,5.605,5.605,0,0,1-5.66,5.66A5.605,5.605,0,0,1,2.264,7.925Z" transform="translate(-18214 -12091)"/></svg>
                    </button>
                    <div class="search-form-holder search-modal cover-modal" data-modal-target-string=".search-modal">
                        <div class="header-search-inner-wrap">
                            <?php get_search_form();?> 
                            <button aria-label="<?php esc_attr_e( 'search form toggle', 'fashion-pin' ); ?>" class="search-icon close" data-toggle-target=".search-modal" data-toggle-body-class="showing-search-modal" data-set-focus=".search-modal .search-field" aria-expanded="false">
...
function blossom_pin_banner(){
    ...
    $banner_title      = get_theme_mod( 'banner_title', __( 'Wondering how your peers are using social media?', 'fashion-pin' ) );
    $banner_subtitle   = get_theme_mod( 'banner_subtitle', __( 'Discover how people in the community create pins to get their attention.', 'fashion-pin' ) );
    $banner_label      = get_theme_mod( 'banner_label', __( 'Discover More', 'fashion-pin' ) );
    ...
    if( $slider_layout == 'one' ){
        ...
        $image_size = 'fashion-pin-slider-two';
    }
        		...
        		<div class="<?php echo esc_attr( $add_class ); ?>banner-layout-<?php echo esc_attr( $slider_layout ); ?> owl-carousel">
                    ...
                    <div class="item">
						...
						<div class="text-holder">
							<?php
                                if( $slider_layout == 'two' ) echo '<div class="holder">';
...
function blossom_pin_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'fashion-pin' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'fashion-pin' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'fashion-pin' );
    ...
    <div class="footer-b">
        <div class="container">
            ...
            <?php
                ...
                esc_html_e( ' Fashion Pin | Developed By ', 'fashion-pin' );
                echo '<a href="' . esc_url( 'https://blossomthemes.com/' ) .'" rel="nofollow" target="_blank">'. esc_html__( 'Blossom Themes', 'fashion-pin' ) . '</a>.';
                
                printf( esc_html__( ' Powered by %s', 'fashion-pin' ), '<a href="'. esc_url( __( 'https://wordpress.org/', 'fashion-pin' ) ) .'" target="_blank">WordPress</a>. ' );
